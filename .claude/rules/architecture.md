...
`DetektConfiguration.kt` sets `buildUponDefaultConfig = true` alongside `disableDefaultRuleSets = true`, which is not the contradiction it looks like and must not be "cleaned up": `disableDefaultRuleSets` is what stops detekt's own rule sets from running, while `buildUponDefaultConfig` is what turns on detekt's config validation. That validation is why a plugin falling off the `detektPlugins` classpath fails the build loudly instead of silently disabling everything it contributed — dropping `io.nlopez.compose.rules` makes every module's detekt task fail with `Property 'Compose' is misspelled or does not exist ... may also indicate a detekt plugin is necessary`, rather than quietly skipping every Compose rule `config/detekt/detekt.yml` configures. Note the limit: validation is rule-set-level, so an unknown rule key *inside* a recognised rule set is accepted without complaint and a rule renamed or removed by an upgrade still has to be caught by reading release notes. With detekt pinned to an alpha, keep both flags as they are.
