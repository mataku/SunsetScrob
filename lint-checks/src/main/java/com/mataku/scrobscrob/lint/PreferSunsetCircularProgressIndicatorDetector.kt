...
class PreferSunsetCircularProgressIndicatorDetector : Detector(), SourceCodeScanner {
  ...
  private fun check(context: JavaContext, node: UImportStatement) {
    ...
    if (fqn != MATERIAL3_CIRCULAR_PROGRESS_INDICATOR_FQCN) return
    ...
    context.report(
      ...
      message = "Use `com.mataku.scrobscrob.ui_common.SunsetCircularProgressIndicator` instead of `androidx.compose.material3.CircularProgressIndicator`.",
    )
  }

  companion object {
    private const val MATERIAL3_CIRCULAR_PROGRESS_INDICATOR_FQCN =
      "androidx.compose.material3.CircularProgressIndicator"
    private const val UI_COMMON_PACKAGE = "com.mataku.scrobscrob.ui_common"

    val ISSUE: Issue = Issue.create(
      id = "PreferSunsetCircularProgressIndicator",
      ...
      explanation = """
        ...
        `com.mataku.scrobscrob.ui_common.SunsetCircularProgressIndicator` wrapper over \
        ...
        has to import material3 `CircularProgressIndicator`, so files in that package are exempt. \
        Suppress with `@Suppress("PreferSunsetCircularProgressIndicator")` only when there is a \
        documented reason.
      """.trimIndent(),
      ...
      implementation = Implementation(
        PreferSunsetCircularProgressIndicatorDetector::class.java,
